...
def update_config(config_id: int, data: EnableOnlyUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    service = NotificationService(db)
    return service.update_config(config_id, data.enabled)

@router.put("/keywords")
def update_keywords(payload: KeywordsUpdateRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    service = NotificationService(db)
    service.update_keywords(current_user.id, payload.keywords)
...
def view_notifications(
    ...
    return NotificationRepository.get_for_user(db, user.id)
